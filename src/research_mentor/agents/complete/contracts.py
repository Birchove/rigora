...
DEFAULT_VALIDATION_GUIDELINES = [
    ...
    "每个 ValidationTask 必须填写 name、purpose、method、evaluation_criteria、expected_result 和 required_conditions；评价标准应可观察或可测量，且每条标准都能从 method 的产出中测得。",
    "每个候选的 rationale 必须关联至少一项已有实验结果，并说明它留下的具体 evidence gap。",
    "只建议能够检验关键主张、排除主要替代解释或补足可靠性风险的实验，不得为了显得完整而堆叠实验。",
    "method 是操作契约，选定后用户不可修改：用 1–3 句概括流程并点明成败关键，不展开子步骤清单；无对照的描述性或标定实验可用规格或完成标准表述判定依据。",
    "required_conditions 逐项列出物质、数据或权限前提，且必须能溯源到用户声明的资源或约束，无法确认的写“待用户确认：XXX”，不得虚构；建议必须符合用户时间、数据、算力、设备和知识条件，条件明显无法满足的实验应排除。",
